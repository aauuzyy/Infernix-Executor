...
// Smart image component: falls back to title initials on 403/error
function ScriptImage({ src, title }) {
  const [failed, setFailed] = useState(false);
  useEffect(() => { setFailed(false); }, [src]);
  if (!src || failed) {
    const initials = (title || '').slice(0, 18) || '?';
    return (
      <div className="thumb-placeholder">
        <div className="thumb-placeholder-initials">{initials}</div>
      </div>
    );
  }
...
function SkeletonCard() {
  return (
    <div className="script-card-skeleton">
...
function ScriptHub({ onLoadScript, onExecuteScript, clients = [] }) {
  ...
  // Debounce refs so placeId briefly going 0 doesn't clear the detected game
  const detectedGameRef   = useRef(null);   // mirrors detectedGame state
  const clearGameTimer    = useRef(null);   // pending timer to null-out game
  const lastDetectedPlaceId = useRef(null); // last non-zero placeId we acted on
  ...
  // Detect game from connected clients — debounced so brief placeId=0 gaps don't reload scripts
  useEffect(() => {
    ...
    if (clientsWithGame.length > 0) {
      // We have a game — cancel any pending clear
      if (clearGameTimer.current) { clearTimeout(clearGameTimer.current); clearGameTimer.current = null; }

      const firstClient = clientsWithGame[0];
      let placeId = Array.isArray(firstClient)
        ? (firstClient[5] || firstClient[6])
        : (firstClient.placeId || firstClient.PlaceId || firstClient.place_id || firstClient.rootPlaceId);
      if (!placeId) return;
      const placeIdStr = String(placeId);

      // Already showing this game — nothing to do
      if (placeIdStr === lastDetectedPlaceId.current) return;
      lastDetectedPlaceId.current = placeIdStr;

      // Cached? Set immediately
      if (gameInfoCache.current.has(placeIdStr)) {
        const info = gameInfoCache.current.get(placeIdStr);
        detectedGameRef.current = info;
        setDetectedGame(info);
        return;
      }

      // Fetch game info (only once per placeId thanks to cache)
      window.electronAPI?.robloxGetGameInfo(placeId).then(gameInfo => {
        if (gameInfo) {
          const fullGameInfo = { placeId: placeIdStr, ...gameInfo };
          gameInfoCache.current.set(placeIdStr, fullGameInfo);
          detectedGameRef.current = fullGameInfo;
          setDetectedGame(fullGameInfo);
        }
      }).catch(err => console.error('Failed to detect game:', err));

    } else {
      // No game visible right now — wait 5 seconds before clearing
      // (handles the gap between Lua polls where placeId briefly drops to 0)
      if (clearGameTimer.current) return; // already waiting
      clearGameTimer.current = setTimeout(() => {
        clearGameTimer.current = null;
        lastDetectedPlaceId.current = null;
        detectedGameRef.current = null;
    ...
    try {
      ...
      if (scriptContent) {
        ...
        if (window.electronAPI?.execute) {
          await window.electronAPI.execute(scriptContent, attachedClients, script.title || 'ScriptHub Script');
        } else {
          ...
          // Record to history when using HTTP fallback
          try {
            await window.electronAPI?.recordExecution?.(script.title || 'ScriptHub Script', scriptContent);
          } catch (_) {}
        }
      }
    ...
    <div className="scripthub">
      <div className="scripthub-header">
        ...
        <div className="search-row">
          <div className="search-box"><Search size={14} /><input type="text" placeholder={detectedGame && gameFilterEnabled ? `Search scripts for ${detectedGame.name}...` : "Search scripts..."} value={search} onChange={(e) => setSearch(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && fetchScripts(true)} /></div>
                ...
                {visibleScripts.map((script, idx) => (
                  <div key={script._id || (startIndex + idx)} className={`script-card fade-in ${script._isUniversal ? 'universal-highlight': ''}`}>
                    <div className="script-thumbnail">
                      <ScriptImage src={getImageUrl(script)} title={script.title} />
                      <div className="thumbnail-overlay">
                        <div className="script-badges">
                          {script.key && <span className="badge key-badge" title="Key Required"><Key size={9} />KEY</span>}
                          {(script.isUniversal || script._isUniversal) && <span className="badge universal-badge" title="Universal Script"><Globe size={9} />UNIVERSAL</span>}
                        </div>
                      </div>
                    </div>
                    <div className="script-content">
                      ...
                      <div className="script-actions">
                        <button className={`sh-btn copy-btn ${copiedId === script._id ? 'copied': ''}`} onClick={() => handleCopyScript(script)} title="Copy Script">
                          <Copy size={12} />
                        </button>
                        <button className="sh-btn open-btn" onClick={() => handleLoadScript(script)} title="Open in Tab">
                          <FileText size={12} />
                          <span>Open</span>
                        </button>
                        <button className="sh-btn execute-btn" onClick={() => handleExecuteScript(script)} title="Execute Script">
                          <Play size={12} />
                          <span>Execute</span>
                        </button>
                      </div>
...
// Only re-render ScriptHub when the detected game (placeId) actually changes.
// Clients update every ~200ms which was causing constant re-renders and lag.
function getClientPlaceId(clients) {
  const c = (clients || []).find(c => {
    const id = Array.isArray(c) ? (c[5] || c[6]) : (c.placeId || c.PlaceId || c.place_id);
...
export default memo(ScriptHub, (prev, next) => {
  return (
    prev.onLoadScript === next.onLoadScript &&
    prev.onExecuteScript === next.onExecuteScript &&
    getClientPlaceId(prev.clients) === getClientPlaceId(next.clients)
  );
});